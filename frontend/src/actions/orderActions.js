...
export const createOrder = (order) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/orders`, order, config);
...
export const getOrderDetails = (id) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/orders/${id}`, config);
